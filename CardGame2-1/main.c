...
const char status_Stack[BOX][20] = {"check", "call", "raise", "fold"}; 
...
//    Get_Mouse_Pos(&x, &y);
    ...
    for(int i = 0; i < BOX; i++)
    {
        ...
        Clicks[i] = Box_Init((i % 2) * (fieldWidth_display - 1) / 2, (i / 2) * (fieldHeight_display - 1) / 2, (fieldWidth_display - 1) / 2, (fieldHeight_display - 1) / 2);
    }
