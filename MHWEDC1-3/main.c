...
int main()
{
    ...
    char weaponType[13] = "LongSword";
    char * typeInput = weaponType;
    int nSlot1 = SLOT1;
    int nSlot2 = SLOT2;
    int nSlot3 = SLOT3;
    float weaponDamageMultiplier = Dictionary(typeInput, &weaponTypeData, damageMultiplier, 13, 14);
...
//                            seriesSkill = findNumArray(MastersTouchKaiser[0], a, 3) + findNumArray(MastersTouchKaiser[1], b, 3) + findNumArray(MastersTouchKaiser[2], c, 3) + findNumArray(MastersTouchKaiser[3], d, 3) + findNumArray(MastersTouchKaiser[4], e, 3);
//                            seriesSkill = ifSame(0, a) + ifSame(0, b) + ifSame(0, c) + ifSame(0, d) + ifSame(0, e);
//                            if(seriesSkill < 4)
//                            {
//                                continue;
//                            }
    ...
    for(int i = HISTORY - 1; i >= 0; i--)
    {
        ...
        printf("Ranking No.%d\nExpectation is %.2f, AttackUp is %d, AffinityUp is %d, Remain nSlot2 is %d, nSlot1 is %d\n", HISTORY - i, ExpectationHistory[i], RankingHistory[i][0][0], RankingHistory[i][0][1], RankingHistory[i][0][2], RankingHistory[i][0][3]);
        printArray2(&RankingHistory[i][1][0], LEVELROWS, LEVELCOLS);
        puts("");
    }
