...
typedef int (*Action)(Game *, float , int);
...
const char status_Stack[BOX][20] = {"info", "check", "call", "raise", "fold"}; 
...
int Betting(Game *game, int player_id, int multi, int c);
...
void Pay_Bet(int *money, int *pot, int *Ready_bet);
...
char kb_m_Input(Box *Clicks, int *x, int *y, int fieldWidth_display, int displayWidth, int displayHeight_Shift, int displayWidth_Shift, int *cnt, int *pre_cnt, int size_box);
...
int Ai_Actions_Callman(Game *game, float prob_win, int nPlayer);
int Ai_Actions_SimRange(Game *game, float prob_win, int nPlayer);
int Ai_Actions_Expec(Game *game, float prob_win, int nPlayer);
Action Ai_Actions[PLAYER] = {Ai_Actions_Callman, Ai_Actions_SimRange, Ai_Actions_Expec};
void save_coin_info(Game * game, int nPlayer, char* coinRecord);
...
int main(int argc, char **argv)
{
    ...
    static char fontData[505][50];
    int fontDataHeight = 505;
    int fontDataWidth = 50;
    static char fontData_Ext[505][50];
    int fontDataHeight_Ext = 505;
    ...
    char coinRecord[50] = "coinRecord.txt";
    FILE *cfPtr = fopen(coinRecord, "w");
    fclose(cfPtr);
    Game Poker;
    memset(&Poker, 0, sizeof(Game));
    
    for(int i = 0; i < PLAYER; i++)
        Poker.players[i].coin = 100000;
    Poker.min_bet = 100;
    Poker.Round_Speed = 4;
    Poker.info = -1;
    Poker.AiIQ = 100;
...
//    srand(2000);
    ...
    kb_m_Input(Clicks, &x, &y, fieldWidth_display, displayWidth, displayHeight_Shitf, 0, &cnt, &pre_cnt, BOX);
    save_coin_info(&Poker, PLAYER, coinRecord);
//    Get_Mouse_Pos(&x, &y);
    pre_cnt = cnt;
    Clicks[0] = Box_Init(500, 18, 130, 54);
    for(int i = 0; i < BOX - 1; i++)
        Clicks[i + 1] = Box_Init((i % 2) * (fieldWidth_display - 1) / 2, (i / 2) * (fieldHeight_display - 1) / 2, (fieldWidth_display - 1) / 2, (fieldHeight_display - 1) / 2);
...
//        Poker.nPlayer_Pos = 0;
        ...
        Poker.range_Ai = 1;
        Poker.shift_Ai = 5;
        for(int i = 0, j = 0; i < PLAYER && j < 2; i++)
            if(Poker.players[(i + Poker.Round) % PLAYER].Error != 5)
                 Poker.blinds[j++] = (i + Poker.Round) % PLAYER;
//        Poker.info = -1;
//Betting time
//Online saving
        ...
        while(1){
            ...
            for(int i = 0; i < PLAYER; i++){
                    ...
                    sprintf(print_field, "Player %d  %d Bet %d", i, Poker.players[i].coin, Poker.players[i].Ready_bet);
...
//Online loading
            ...
            if((player_sh->Error = Betting(&Poker, player_id, multi == 0 ? -1 : Poker.nPlayer_Pos, Poker.blinds[0] == Poker.nPlayer_Pos ? 'n' : 
                                                                                                    Poker.blinds[1] == Poker.nPlayer_Pos ? 'b' : ' ')) >= 0){
...
//Online saving
        ...
        Poker.Total_Call += Poker.call;
        for(int i = 0; i < PLAYER; i++)
            Pay_Bet(&Poker.players[i].coin, &Poker.pot, &Poker.players[i].Ready_bet);
...
//Change Cards
        ...
        {
            Poker.nPlayer_Pos = Poker.Round % PLAYER;
            for(int i = 0; i < PLAYER; i++){
                if(Poker.players[i].Error == 5)
                    continue;
                Poker.players[i].Error = 0;
...
//Blind         
                for(int i = 0; i < 2; i++)
                    Letter_Draw_Normal(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, Card_DesignWidth * (2 * Poker.blinds[i] + 1), Card_Height + font_size * 3 + 2, i == 0 ? "SB" : "BB", font_size);
...
//Stage & Pot
                ...
                memset(print_field, 0, 50);
                sprintf(print_field, "Called %d (%d)", Poker.Total_Call, Poker.Total_Call + Poker.call);
...
//Online loading
                    ...
                    if((player_sh->Error = Betting(&Poker, player_id, multi == 0 ? -1 : Poker.nPlayer_Pos, 
                                    ...
                                    kb_m_Input(Clicks, &x, &y, fieldWidth_display, displayWidth, displayHeight_Shitf, 0, &cnt, &pre_cnt, BOX) :  
                                                                    Ai_Actions[Poker.nPlayer_Pos / 2](&Poker, player_sh->prob_win / 100, PLAYER)))>= 0)
...
//Here online uplink
        ...
        for(int i = 0; i < PLAYER; i++)
            Poker.players[i].Hand_id = Hnds_Idntfy(Poker.players[i].Tb_Hd_Hands, HAND + (TBL == 0 ? 0 : Poker.stage));
...
//Stage & Pot
        ...
        Letter_Draw_Normal(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, Card_DesignWidth * 5, 0, print_field, font_size);
        memset(print_field, 0, 50);
        sprintf(print_field, "Total Call %d", Poker.Total_Call);
        Letter_Draw_Normal(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, Card_DesignWidth * 5, font_size, print_field, font_size);
        memset(print_field, 0, 50);
...
//Info Display
        if(TBL == 0){
            Odds_Prob_Draw(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, Poker.pot, 0, Card_Height + font_size*4 + 2, font_size);
...
//        won = Hand_Cmp(Poker.players, PLAYER, HAND);
        ...
        counter_temp = 0;
        Poker.Round++;
        Poker.min_bet += Poker.Round % Poker.Round_Speed == 0 ? pow(10, floor(log10(Poker.min_bet))) : 0;
        Poker.nPlayer_Pos = Poker.Round % PLAYER;
        Poker.Total_Call = 0;
    }
    memset(field, 0, fieldWidth * fieldHeight);
    for(int i = 0; i < PLAYER; i++){
        memset(print_field, 0, 50);
        sprintf(print_field, "Player %d   %d wons", i+1, Poker.won_count[i]);
        Letter_Draw_Normal(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, 0, font_size * i, print_field, font_size);
    }
    system("cls");
    printArray3(fieldHeight, fieldWidth, field);
	return 0;
...
{   
    ...
    int *pot = &game->pot;
    int *Ready_bet = &player_sh->Ready_bet;
    int *info = &game->info;
    ...
    {//All of them --check , next round
        ...
        else if((c - 48) * min_bet == *call){
            *Ready_bet = (*call);
            *bet = (*call);
            *status = *lead;
            return 2; //call
        }
        else if((c - 48) * min_bet >= *call * 2){
            *Ready_bet = min_bet * (c - 48);
            *bet = min_bet * (c - 48);
            (*call) = *bet;
            (*lead)++;
...
void Pay_Bet(int *money, int *pot, int *Ready_bet)
{
    *pot += *Ready_bet < *money ? *Ready_bet : *money;
    *money = *Ready_bet < *money ? *money - *Ready_bet : 0; 
...
int Ai_Actions_SimRange(Game *game, float prob_win, int nPlayer)
{
    int range_Ai = game->range_Ai;
    int shift_Ai = game->shift_Ai;
    int call = game->call;
    int min_bet = game->min_bet;
    prob_win *= 100;
    if(prob_win < (float)100 / (nPlayer + range_Ai) + shift_Ai)
        return 'f';
    if(prob_win > (float)100 / (nPlayer + range_Ai) * (nPlayer + range_Ai - 1) + shift_Ai && call < min_bet * pow(2, 3)) 
        return 'r';
    return 'c';
}
...
int Ai_Actions_Expec(Game *game, float prob_win, int nPlayer)
{
    ...
    for(int i = 0; i < nPlayer; i++)
        pot += game->players[i].Ready_bet;
    
    int n = nPlayer - 1 - Status_count(&game->players, nPlayer, 5);
    
    int a = 80;
    int a1 = 14;
    double c = -1.16;
    double d = 2.5;
    double bet_original = pot * prob_win / (1 - prob_win);
    double ATAN = atan(bet_original / min_bet / (a1 * n) + c);
    int bet = floor(((1 - ((2 / M_PI) * ATAN)) * bet_original / min_bet) / d) * min_bet;
   
//    double ATAN = atan(bet_original / min_bet / a + c);
//    int bet = floor(((1 - ((2 / M_PI) * ATAN)) * bet_original / min_bet) / d) * min_bet;
   
    if(call == 0 && bet_original < min_bet)
        return ' ';
    if(bet_original < call || bet_original < min_bet)
        return 'f';
    
    if(bet < call * 2 || call == 0)
        return 'c';
    return bet * 10;
}

void save_coin_info(Game * game, int nPlayer, char *fname)
{
    FILE *cfPtr;
