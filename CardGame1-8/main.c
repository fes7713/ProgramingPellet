...
#define FACE_EX 14
#define MULTI 0
#define PPI_ Poker.players[i].
#define PPPN_ Poker.players[Poker.nPlayer_Pos].
#define PPP_ Poker.players[player_id].
#define PNP_ Poker.nPlayer_Pos
#define FFFF_EXT fontDataWidth_Ext, fontData_Ext, fieldWidth, field
#define FFFF fontDataWidth, fontData, fieldWidth, field
#define CCFF Card_DesignWidth, Card_Design, fieldWidth, field
typedef struct{
    char * face;
    char * suit;
    int num;
...
int main(int argc, char **argv)
{
    ...
    for(int i = 0; i < PLAYER; i++)
        Cpy_Hnds(deck, PPI_Hands, HAND, posHand[i]);
    Cpy_Hnds(deck, Poker.TB_Hands, TBL, posTable);
    
    for(int i = 0; i < PLAYER; i++)
        Hand_cat(PPI_Tb_Hd_Hands, PPI_Hands, Poker.TB_Hands, HAND, 0);
    Player *self = &Poker.players[player_id];

    while(1){
        self->bet = 0;
        for(int i = 0; i < PLAYER; i++){
            strcpy(&PPI_name, names[i]);
            PPI_bet = 0;
            if(PPI_coin < Poker.min_bet)
                PPI_Error = 5;
            else
                PPI_Error = 0;
            PPI_Total_bet = 0;
...
//Online saving
        ...
        while(1){
            ...
            if(TBL == 0)
                Odds_Prob_Draw(FFFF_EXT, self->bet, 0, Card_Height + font_size*4 + 2, font_size);
            for(int i = 0; i < PLAYER; i++){
                    memset(print_field, 0, 50);
                    sprintf(print_field, "Player %d  %d Bet %d", i, PPI_coin, PPI_Total_bet);
                    Letter_Draw_Normal(FFFF_EXT, Card_DesignWidth * 2 * i, Card_Height + font_size  + 2, print_field, font_size);
                    Mny_Bt_Draw(FFFF_EXT, -1, PPI_bet, -1, Poker.call, &PPI_Error, Card_DesignWidth * 2 * i, Card_Height + font_size*2 + 2, font_size);
            }
            
            if(player_id == PNP_ && TBL > 0)
                BG_Draw(CCFF, Card_DesignWidth * 2 * PNP_, Card_Height + 2 + font_size / 3, Card_Height, HAND, font_size / 3);
            Letter_Draw_Normal(FFFF_EXT, Card_DesignWidth * 2 * PNP_ + Card_DesignWidth * HAND / 2 - font_size / 2 * 6, Card_Height + 2, "Play", font_size);
//Stage & pot
            memset(print_field, 0, 50);
            sprintf(print_field, "Stage %d Pot %d", Poker.stage,  Poker.pot);
            Letter_Draw_Normal(FFFF_EXT, Card_DesignWidth * 5, 0, print_field, font_size);
            memset(print_field, 0, 50);
            sprintf(print_field, "Player_Pos %d", PNP_);
            Letter_Draw_Normal(FFFF_EXT, Card_DesignWidth * 5, font_size, print_field, font_size);
            memset(print_field, 0, 50);
            sprintf(print_field, "Lead %d", Poker.lead);
            Letter_Draw_Normal(FFFF_EXT, Card_DesignWidth * 5, font_size * 2, print_field, font_size);
            memset(print_field, 0, 50);
            sprintf(print_field, "Minimum_bet %d", Poker.min_bet);
            Letter_Draw_Normal(FFFF_EXT, Card_DesignWidth * 5, font_size * 3, print_field, font_size);
            system("cls");
//            deal(deck);
            
            printArray3(fieldHeight, fieldWidth, field);
            Player * player_sh = &Poker.players[PNP_];
//Online loading
            ...
            if((player_sh->Error = Betting(&Poker, player_id, multi == 0 ? -1 : PNP_, PNP_ < HMN ? 0 : 'c')) >= 0){
                for(int i = 0; i < PLAYER; i++)
                    if(PPI_Error == 5)
...
//Online saving
        ...
        for(int i = 0; i < PLAYER; i++)
            Pay_Bet(&PPI_coin, &Poker.pot, &PPI_Total_bet);
...
//Table Card Drawing
                if(TBL > 0){
                    nTable_Draw(CCFF, 0, 0, Poker.TB_Hands, TBL, Poker.stage, Card_Height);
                    for(int i = 0; i < PLAYER; i++)
                        nTable_Draw(CCFF, Card_DesignWidth * 2 * i, Card_Height + font_size * 4 + 2, PPI_Hands, HAND, player_id == i ? 2 : 0, Card_Height);
                    if(player_id == PNP_)
                        BG_Draw(CCFF, Card_DesignWidth * 2 * PNP_, Card_Height + 2 + font_size / 3, Card_Height, HAND, font_size / 3);
                    Letter_Draw_Normal(FFFF_EXT, Card_DesignWidth * 2 * PNP_ + Card_DesignWidth * HAND / 2 - font_size / 2 * 6, Card_Height + 2, "Play", font_size);
                }
                else
                    nTable_Draw(CCFF, 0, 0, PPP_Hands, HAND, HAND, Card_Height);
                
                for(int i = 0; i < PLAYER; i++){
                    memset(print_field, 0, 50);
                    sprintf(print_field, "Player %d  %d Bet %d", i, PPI_coin, PPI_Total_bet);
                    Letter_Draw_Normal(FFFF_EXT, Card_DesignWidth * 2 * i, Card_Height + font_size  + 2, print_field, font_size);
                    Mny_Bt_Draw(FFFF_EXT, -1, PPI_bet, -2, Poker.call, &PPI_Error, Card_DesignWidth * 5, font_size + font_size * 2 * (i + 1), font_size);
                    Mny_Bt_Draw(FFFF_EXT, -1, PPI_bet, -1, Poker.call, &PPI_Error, Card_DesignWidth * 2 * i, Card_Height + font_size*2 + 2, font_size);
                }

//Stay or Change
                if(TBL == 0 || Poker.info == player_id){
                    S_C_Draw(FFFF, Chng_Pos, HAND, 0, Card_Height + 1, font_size, Card_DesignWidth);
                    Odds_Prob_Draw(FFFF_EXT, Poker.info != -1 ? Poker.min_bet :  Poker.pot, Poker.info != -1 ? Card_DesignWidth * 2 * ((player_id + 1) % PLAYER) : 0, Card_Height + font_size*4 + 2, font_size);
                }
                
//Hands Rank
                Letter_Draw_Normal(FFFF, Card_DesignWidth * 2 * player_id, Card_Height + font_size * 3 + 2, Hands_Rank[self->Hand_id.hand], font_size);
//Stage & Pot
                memset(print_field, 0, 50);
                sprintf(print_field, "Stage %d Pot %d", Poker.stage,  Poker.pot);
                Letter_Draw_Normal(FFFF_EXT, Card_DesignWidth * 5, 0, print_field, font_size);
...
//Online saving
            ...
            if(Poker.stage == 0 && TBL > 0)
                ...
                if(Poker.stage < 5 && TBL > 0)
                    Poker.stage++;
            for(int i = 0; i < PLAYER; i++)
                Hand_cat(PPI_Tb_Hd_Hands, PPI_Hands, Poker.TB_Hands, HAND, Poker.stage);
        ...
        if(TBL == 0)
            Hand_cat(PPP_Tb_Hd_Hands, PPP_Hands, Poker.TB_Hands, HAND, 0);
            
        for(int i = 0; i < PLAYER; i++)
            PPI_Hand_id = Hnds_Idntfy(PPI_Tb_Hd_Hands, HAND + Poker.stage);
...
//Online saving
        ...
        if(TBL != 0){
            nTable_Draw(CCFF, 0, 0, Poker.TB_Hands, TBL, Poker.stage, Card_Height);
            for(int i = 0; i < PLAYER; i++)
                nTable_Draw(CCFF, Card_DesignWidth * 2 * i, Card_Height + font_size * 4 + 2, PPI_Hands, HAND, 2, Card_Height);
        }
        else{
            nTable_Draw(CCFF, 0, 0, PPP_Hands, HAND, HAND, Card_Height);
//Info Display
            Odds_Prob_Draw(FFFF_EXT, Poker.pot, 0, Card_Height + font_size*4 + 2, font_size);
            Mny_Bt_Draw(FFFF_EXT, self->coin, Poker.pot, Poker.pot * odds[self->Hand_id.hand], -1, &self->Error, 0, Card_Height + font_size*2 + 2, font_size);
        }
        ...
        for(int i = 0; i < PLAYER; i++){
            ...
            Letter_Draw_Normal(FFFF_EXT, Card_DesignWidth * 2 * i, Card_Height + font_size  + 2, print_field, font_size);
            Mny_Bt_Draw(FFFF_EXT, -1, PPI_bet, -1, Poker.call, &PPI_Error, Card_DesignWidth * 2 * i, Card_Height + font_size*2 + 2, font_size);
            Letter_Draw_Normal(FFFF, Card_DesignWidth * 2 * i, Card_Height + font_size * 3 + 2, Hands_Rank[PPI_Hand_id.hand], font_size);
        }  
...
//        if((multi == 1 && player_id == 0) || multi == 0)
        ...
        if(TBL > 0){
            ...
            for(int i = 0; i < PLAYER; i++)
                if(PPI_Error != 5)
                    Get_Award_Ext(&PPI_coin, Poker.min_bet, PPI_Hand_id.hand, odds_Ext);
        }
        else
            Get_Award(&PPP_coin, &Poker.pot, PPP_Hand_id.hand);
        ...
        Cpy_Hnds(deck, Poker.TB_Hands, TBL, posTable);
        for(int i = 0; i < PLAYER; i++){
            Hand_cat(PPI_Tb_Hd_Hands, PPI_Hands, Poker.TB_Hands, HAND, TBL);
            PPI_Error = 0;
        }
        counter_temp = 0;
        Poker.min_bet += pow(10, floor(log10(Poker.min_bet)) - 1);
    }
...
void S_C_Draw(int fontDataWidth, char (*fontData)[fontDataWidth], int fieldWidth, char (*field)[fieldWidth], bool *Chng_Pos, int size, int DrawFromX, int DrawFromY, int letter_size, int Card_DesignWidth)
{
    ...
    Letter_Draw_Normal(FFFF, DrawFromX, DrawFromY, print_string, letter_size);
}
...
void Odds_Prob_Draw(int fontDataWidth, char (*fontData)[fontDataWidth], int fieldWidth, char (*field)[fieldWidth], int bet, int DrawFromX, int DrawFromY, int letter_size)
{
    ...
    for(int i = 0; i < 10; i++)
    {
        ...
        Letter_Draw_Normal(FFFF, DrawFromX, DrawFromY + letter_size * i, print_field, letter_size);
    }
}
...
void Mny_Bt_Draw(int fontDataWidth, char (*fontData)[fontDataWidth], int fieldWidth, char (*field)[fieldWidth], int money, int bet, int earned, int call, int *Error, int DrawFromX, int DrawFromY, int letter_size)
{
    ...
    if(*Error == -5)
        strcat(print_field, "Not Your Turn");
    if(call != 0){
        if(*Error == -6)
...
//            sprintf(print_field, "%s %d To Call", print_field, call);
    ...
    Letter_Draw_Normal(FFFF, DrawFromX, DrawFromY, print_field, letter_size);
//    *Error = 0;
}
