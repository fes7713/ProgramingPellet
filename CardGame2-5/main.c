...
Action Ai_Actions[5] = {Ai_Actions_SimRange, Ai_Actions_Expec};
...
//            if((player_sh->Error = Betting(&Poker, player_id, multi == 0 ? -1 : Poker.nPlayer_Pos, Poker.nPlayer_Pos < HMN ? 0 : 'c')) >= 0){
            ...
            if((player_sh->Error = Betting(&Poker, player_id, multi == 0 ? -1 : Poker.nPlayer_Pos, Poker.Round == Poker.nPlayer_Pos ? 'n' : 
                                                                                                    Poker.Round + 1 == Poker.nPlayer_Pos ? 'b' : ' ')) >= 0){
...
//Blind         
                for(int i = 0; i < 2; i++)
                    Letter_Draw_Normal(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, Card_DesignWidth * (2 * ((Poker.Round  + i) % PLAYER) + 1), Card_Height + font_size * 3 + 2, i == 0 ? "SB" : "BB", font_size);
...
//                                                                                                                                                                                  : 'c')) >= 0)
                    if((player_sh->Error = Betting(&Poker, player_id, multi == 0 ? -1 : Poker.nPlayer_Pos, 
                                    ...
                                    Poker.nPlayer_Pos < HMN ? 
                                    kb_m_Input(Clicks, &x, &y, fieldWidth_display, displayWidth, displayHeight_Shitf, 0, &cnt, &pre_cnt, BOX) :  
                                                                    Ai_Actions[1](&Poker, player_sh->prob_win / 100, PLAYER)))>= 0)
