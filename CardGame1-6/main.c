...
int Betting(int *bet, int *money, int min_bet, int *call, int *lead, int *status, int *pot, int *Total_bet, int * info, int player_id,  char c);
...
//                    Mny_Bt_Draw(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, -1, Poker.players[i].bet, -2, Poker.call, &Poker.players[i].Error, Card_DesignWidth * 5, font_size + font_size * 2 * (i + 1), font_size);
            ...
            Letter_Draw_Normal(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, Card_DesignWidth * 2 * Poker.nPlayer_Pos, Card_Height + 2, "       Play", font_size);
//Stage & pot
            memset(print_field, 0, 50);
...
//Online loading
            ...
            if((player_sh->Error = Betting(&player_sh->bet, &player_sh->coin, Poker.min_bet, &Poker.call, &Poker.lead, &Poker.status[Poker.nPlayer_Pos], &Poker.pot, &player_sh->Total_bet, &Poker.info, player_id, Poker.nPlayer_Pos < HMN ? 0 : 'c')) >= 0){
                for(int i = 0; i < PLAYER; i++)
...
//                    return -6;
//        case 's': if(*Total_bet < *bet - 100) 
//                        (*bet) = range((*bet) - pow(10, floor(log10((*bet))) - 1));
//                    *status = -7;
//                    return -7;
        case 'd': if(*bet == 0)
                        *bet = min_bet * 2;
                    ...
                    return -6;
        case 'a': if(*Total_bet < *bet / 2) 
                        (*bet) = range((*bet) / 2);
                    *status = -7;
                    return -7;
        case 'i': if(*info == player_id)
                    *info = -1;
                    else 
                        *info = player_id;
                    return -7;
