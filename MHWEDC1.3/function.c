...
float Dictionary(char *str ,char *arrKey, float arrnum[], int nSizeLetter, int nSize)
{
    int i, j;
    char ** arrKeyHold;
    char ** temparrKey;
    ...
    float result = arrnum[0];
    
    arrKeyHold = (char **)malloc(sizeof(char *) * nSize);
    strHold = (char *)malloc(sizeof(char) * nSizeLetter);
    
    for(i = 0; i < nSize; i++)
    {
        arrKeyHold[i] = (char *)malloc(sizeof(char) * nSizeLetter);
    }
    
    for(i = 0; i < nSize; i++)
    {
        for(j = 0; j < nSizeLetter; j++)
        {
            temparrKey = arrKey + i * nSizeLetter + j;
            arrKeyHold[i][j] = * temparrKey;
        }
    }
    
    for(i = 0; i < nSizeLetter; i++)
    {
        strHold[i] = *str;
        str++;
    }
...
void swapArray2(float * arrNum1, float * arrNum2, int rows, int cols)
{
    for(int i = 0; i < rows; i++)
    {
        for(int j = 0; j < cols; j++)
...
void RankingWithHistory13(int size, int rows, int cols, float RankingInput, int HistoryInput[][cols], float RankingArr[], int HistoryArr[][rows + 1][cols])
{
    if(RankingInput > RankingArr[0])
    {
        RankingArr[0] = RankingInput;
        ...
        for(int i = 0; i < size - 1; i++)
        {
            ...
            {
                swap(&RankingArr[i], &RankingArr[i + 1]);
                swapArray2(&HistoryArr[i], &HistoryArr[i + 1], rows + 1, cols);
            }
            else{
                break;
            }
