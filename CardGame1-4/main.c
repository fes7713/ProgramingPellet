...
void nTable_Draw(int fontDataWidth, char (*fontData)[fontDataWidth], int fieldWidth, char (*field)[fieldWidth], int DrawFromX, int DrawFromY, int DrawFromX_Tb, int DrawFromY_Tb, Card *Hand, Card *Table, int size, int Tb_size, int Tb_Open, int letter_size);
...
int Betting(int *bet, int *money, int min_bet, int *call, int *raise, int *pot, int *Total_bet);
...
void Pay_Bet(int *money, int bet, int *pot, int *Total_bet);
void Get_Award(int *money, int bet, int Hand_id);
void Get_Pot(int *money, int bet, int *pot, int Hand_id);
...
int main(int argc, char **argv)
{
    ...
    int multi = 0;
    Game Poker;
    Poker.players[player_id].coin = 5000;
...
//Stage & pot
            ...
            if(Poker.stage > 0)
                nTable_Draw(Card_DesignWidth, Card_Design, fieldWidth, field, 0, 0, Card_DesignWidth * 2, Card_Height + font_size * 4 + 2, self->Hands, Poker.TB_Hands, HAND, TBL, Poker.stage, Card_Height);
            system("cls");
//            deal(deck);
            printArray3(fieldHeight, fieldWidth, field);
            if((self->Error = Betting(&self->bet, &self->coin, Poker.min_bet, &Poker.call, &Poker.raise, &Poker.pot, &self->Total_bet)) == 6)
                break;
        }
...
//Stage & Pot
                ...
                for(int i = 0; i < PLAYER; i++){
                    memset(print_field, 0, 50);
                    sprintf(print_field, "Player %ld", i);
                    Letter_Draw_Normal(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, Card_DesignWidth * 5, font_size * 2 * (i + 1), print_field, font_size);
...
//Texus Hodem
                    ...
                    if(self->Error != 6)
                        if((self->Error = Betting(&self->bet, &self->coin, Poker.min_bet, &Poker.call, &Poker.raise, &Poker.pot, &self->Total_bet)) == 6)
                        {
                            if(multi == 0){
                //Auto call    
                                Poker.players[!(player_id)].bet = Poker.call;
                                Poker.players[!(player_id)].Error = 2;
                                Pay_Bet(&Poker.players[!(player_id)].coin, Poker.players[!(player_id)].bet, &Poker.pot, &Poker.players[!(player_id)].Total_bet);
                            }
                            Poker.nCall = 0;
                            for(int i = 0; i < PLAYER; i++)
                                if(Poker.players[i].bet == Poker.call)
                                    Poker.nCall++;
                            if(Poker.nCall == PLAYER)
                                break;
                        }
                }
//Here online uplink
        ...
        self->Hand_id = Hnds_Idntfy(self->Tb_Hd_Hands, HAND + Poker.stage);
        Poker.players[!(player_id)].Hand_id = Hnds_Idntfy(Poker.players[!(player_id)].Tb_Hd_Hands, HAND + Poker.stage);
        
//        if(self->Hand_id.hand == 4){
            memset(field, 0, fieldWidth * fieldHeight);
            nTable_Draw(Card_DesignWidth, Card_Design, fieldWidth, field, 0, 0, Card_DesignWidth * 2, Card_Height + font_size * 4 + 2, self->Hands, Poker.TB_Hands, HAND, TBL, Poker.stage, Card_Height);
//Opponent card
            if(PLAYER > 1){//////// Has to be no more than 2;
                nTable_Draw(Card_DesignWidth, Card_Design, fieldWidth, field, Card_DesignWidth * 4, Card_Height + font_size * 4 + 2, 0, 0, Poker.players[!(player_id)].Hands, Poker.TB_Hands, HAND, 0, Poker.stage, Card_Height);
                Letter_Draw_Normal(fontDataWidth, fontData, fieldWidth, field, 150, Card_Height + font_size + 2, Hands_Rank[Poker.players[!(player_id)].Hand_id.hand], font_size);
//Opponent Reference info Draw
                memset(print_field, 0, 50);
                sprintf(print_field, "Ref1 %d", Poker.players[!(player_id)].Hand_id.ref_1 + 1);
                Letter_Draw_Normal(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, Card_DesignWidth * 4.75, Card_Height + font_size + 2, print_field, font_size);
                memset(print_field, 0, 50);
                sprintf(print_field, "Ref2 %d", Poker.players[!(player_id)].Hand_id.ref_2 + 1);
                Letter_Draw_Normal(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, Card_DesignWidth * 4.75, Card_Height + font_size*2 + 2, print_field, font_size);
                memset(print_field, 0, 50);
                sprintf(print_field, "Ref3 %d", Poker.players[!(player_id)].Hand_id.ref_3 + 1);
                Letter_Draw_Normal(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, Card_DesignWidth * 4.75, Card_Height + font_size*3 + 2, print_field, font_size);
            }
            Letter_Draw_Normal(fontDataWidth, fontData, fieldWidth, field, 0, Card_Height + font_size + 2, Hands_Rank[self->Hand_id.hand], font_size);
...
//            Odds_Prob_Draw(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, bet, Card_DesignWidth * HAND, 0, font_size);
            ...
            Mny_Bt_Draw(fontDataWidth_Ext, fontData_Ext, fieldWidth, field, self->coin, self->bet, self->bet * odds[self->Hand_id.hand], -1, &self->Error, 0, Card_Height + font_size*2 + 2, font_size);
            system("cls");
            deal(deck);
            printArray3(fieldHeight, fieldWidth, field);
...
//        Get_Award(&self->coin, self->bet, self->Hand_id.hand);
        Get_Pot(&self->coin, self->bet, &Poker.pot, self->Hand_id.hand);
        Poker.stage = 0;
        shuffleDeck(deck);
        Draw_Start = 0;
        Drawer(HAND, PLAYER, posHand, &Draw_Start);
...
//        strcpy(&field[DrawFromY + i][DrawFromX], string_field);
    ...
    if(Tb_size > 0){
        for(int i = 0; i < letter_size; i++) // 5 is letter hight
        {
            ...
            for(int j = 0; j < Tb_size - Tb_Open; j++)
...
//        case 'a': *bet = *money, *allin = *money; return 4;
        ...
        default : 
                    if((*bet) < min_bet || (*bet) > *money || (*bet) < *call)
                    {
                        if((*bet) < min_bet)
                            return -1;
                        if((*bet) < *call)
                    ...
                    Pay_Bet(money, *bet, pot, Total_bet);
                    return 6;
    }
}
